...
> [!info] Context: These are Python questions for a senior backend/DevOps engineer who uses Python for automation, scripting, and tooling — not a Python-only or data science role. Short answers, rapid-fire format.
...
`queue.Queue` is thread-safe, blocks on get/put. Used in threading. `asyncio.Queue` is for coroutines, uses `await` for get/put. Used in async code. Don't mix them — using `queue.Queue` in async code blocks the event loop. In CDeploy scripts, I use `asyncio.Queue` for coordinating async tasks (deploy to multiple environments concurrently).
...
For simple scripts: environment variables via `os.environ` + `.env` files via `python-dotenv`. For complex tools: `click` library for CLI args + YAML config files via `ruamel.yaml`. In CDeploy at Combination, we use `environments.py` with environment-specific settings + CLI wrappers. Never hardcode config — always externalize.
...
pyproject.toml
```
Use `pyproject.toml` (PEP 621) instead of `setup.py`. Type hints everywhere. `pytest` for testing. `black` for formatting, `ruff` for linting, `mypy` for type checking. At Volvo, our Python scripts followed this structure with `flake8` and `mypy` enforced in the ZUUL CI pipeline.
...
> [!faq]- "How do you use Python in your daily work?"
> Automation and tooling, not application development. At Combination, I maintain the CDeploy framework — Python orchestrating AKS clusters, DNS, Kafka topics, K8s resources. At Volvo, I wrote the Gerrit trigger daemon, build orchestration, and manifest management in Python. At Toyota, Python test frameworks for fleet simulation. I use Python when I need to glue systems together — it's the best scripting language for DevOps/automation.

> [!faq]- "Python vs C# — when do you choose which?"
> C# for application code — microservices, APIs, business logic. Type safety, performance, rich ecosystem for backend. Python for automation, scripting, and tooling — fast to write, easy to read, great for gluing systems together. At Combination: C# for the 60+ microservices, Python for CDeploy infrastructure automation. Different tools, different jobs.

> [!faq]- "How do you handle errors in Python scripts?"
> Specific exceptions, not bare `except:`. Log the error with context (what operation, what input). For automation scripts: fail fast and loud — if a deployment step fails, stop and alert, don't continue with a broken state. Use `try/except/finally` for cleanup (close connections, release resources). In CDeploy, each operation has retry logic with exponential backoff for transient failures (network, API rate limits).

> [!faq]- "How do you test Python code?"
> `pytest` for everything. Fixtures for test setup (database connections, temp files). `unittest.mock.patch` for mocking external calls. At Volvo, `flake8` + `mypy` + `pytest` enforced in the ZUUL CI pipeline. At Combination, Python tests for CDeploy run in GitHub Actions. Structure: `tests/` directory mirroring `src/`, one test file per module.
