...
tags:
  - education-kit
---
# C# / .NET — Knowledge Base
> [!info] Comprehensive C# and .NET interview questions covering language fundamentals, async patterns, LINQ, DI, memory management, ASP.NET Core, EF Core, gRPC, SignalR, and modern .NET 8/9 features.
...
`Channel<T>` is a thread-safe, bounded or unbounded queue for async producer-consumer scenarios. Producer writes with `channel.Writer.WriteAsync()`, consumer reads with `channel.Reader.ReadAllAsync()`. Backpressure is built in for bounded channels — producer blocks when the channel is full. This pattern is useful for background processing — incoming requests write to a channel, a background worker processes them sequentially.
...
It tells EF Core to skip the change tracker for the returned entities, reducing memory and CPU overhead. Use it for read-only queries (GraphQL resolvers, read endpoints) where you never intend to call `SaveChanges()`.
...
`Span<T>` is a stack-allocated, ref-like struct that provides a memory-safe view over contiguous memory (arrays, strings, native buffers) without copying. It enables zero-allocation parsing and slicing — e.g., parsing a large JSON payload without allocating substrings. `Span<T>` cannot be stored on the heap (no fields, no async methods); use `Memory<T>` when you need heap storage.
...
Pattern matching replaces verbose if-else chains with concise `switch` expressions. You can combine type patterns, property patterns, relational patterns (`> 0 and < 100`), and list patterns (`[_, .., var last]`). Practical use: validating request fields or mapping resolver inputs to domain commands.
...
> Constructor injection for dependencies (interface-based), avoid static methods with side effects, keep business logic in domain classes (not controllers), return Task for anything async. The pattern: interface in ApplicationCore, implementation in Infrastructure, test mocks the interface. WebApplicationFactory for in-process testing, NSubstitute for mocking external dependencies.
